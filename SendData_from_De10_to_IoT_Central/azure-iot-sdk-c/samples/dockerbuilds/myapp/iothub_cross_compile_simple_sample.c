...
static const char* connectionString = "HostName=iotc-e8d8185c-5bd4-42e0-95b8-08059ad42e02.azure-devices.net;DeviceId=iotde10standard;SharedAccessKey=2vtRILQRpDhYiuCsR0w28kOTCZ/N8/Hd7aQnxUodTPY=";

#define MESSAGE_COUNT        1
int g_interval = 5000;  // 5 sec send interval initially
...
int main(void)
{
    ...
    (void)printf("Creating IoTCentral handle\r\n");
    // Create the iothub handle here
    device_handle = IoTHubDeviceClient_CreateFromConnectionString(connectionString, protocol);
    
	if (device_handle == NULL)
    {
        (void)printf("Failure creating IotCentral device. Hint: Check your connection string.\r\n");
    }
    else
    {
		#ifdef SET_TRUSTED_CERT_IN_SAMPLES
			...
			while(i--)
			{
				...
				telemetry_temperature = data[0].value;
				telemetry_humidity = data[1].value ;
				led_1 = data[2].value;
				led_2 = data[3].value;

				printf(telemetry_msg_buffer, "{\"Temperature\":%.2f,\"Humidity\":%.2f, \"Led1\":%.0f, \"Led2\":%.0f}",
					telemetry_temperature, telemetry_humidity, led_1, led_2);

				sprintf(telemetry_msg_buffer, "{\"temperature\":%.2f,\"humidity\":%.2f, \"led1\":%.0f, \"led2\":%.0f}",
					telemetry_temperature, telemetry_humidity, led_1, led_2);

				message_handle = IoTHubMessage_CreateFromString(telemetry_msg_buffer);

				(void)printf("\r\nSending message %d to IoTCentral \r\nMessage: %s\r\n", (int)(messagecount + 1), telemetry_msg_buffer);
